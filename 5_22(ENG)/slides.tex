...
\title[]{Survey of Sequential Clinical Diagnosis with MIMIC-CDM}
\subtitle{Focusing on Path Supervision and Stopping}
\author[]{SeoJun Ha}
\institute{DGIST}
\date{May 22, 2026}
...
\begin{itemize}
\small
  \item \textbf{LA-CDM}: RL that optimizes diagnostic accuracy and test cost
  \item \textbf{LDTL}: treats the full trajectory as latent and approximates it with one-step information gain
  \item \textbf{SIFT}: RL with intermediate rewards for retrieving more high-quality evidence
\end{itemize}
\end{frame}
...
\vspace{0.1em}
\begin{block}{1. LLM diagnosis does not explicitly model test selection}
\small
LLM-based diagnostic systems can generate diagnoses, but they do not explicitly model \textbf{which tests should be selected and in what order}, as in real clinical pathways.
\end{block}

\vspace{0.35em}
\begin{block}{2. Terminal reward is insufficient for trajectory learning}
\small
...
\small
\textbf{Teacher agent}: generates action-wise diagnostic improvement signals from latent trajectories.

\vspace{0.2em}
\textbf{Student agent}: learns the test selection policy using the teacher signal.
\end{block}
\end{frame}
...
\begin{frame}[t]{SIFT: Results and Implications}
\centering
\vspace{0.25em}
...
\vspace{0.45em}
{\footnotesize Interpretation: the critical evidence ratio explains diagnostic success more strongly than the amount of evidence.}
\end{frame}
