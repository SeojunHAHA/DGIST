...
{\footnotesize HA가 낸 hypothesis와 confidence를 보고, 다음 test를 볼지 diagnosis를 낼지 학습한다.}

\vspace{0.45em}
\textbf{HA SFT}

{\footnotesize 현재까지 관측된 patient state에서 정답 diagnosis hypothesis를 생성하도록 학습한다.}

\vspace{0.45em}
\textbf{HA confidence GRPO}

{\footnotesize 맞는 hypothesis는 높은 confidence, 틀린 hypothesis는 낮은 confidence를 내도록 보상한다.}

\vspace{0.55em}
{\scriptsize\textcolor{gray!75}{DA GRPO \(\rightarrow\) HA SFT \(\rightarrow\) HA confidence GRPO를 반복한다.}}
\end{block}
\end{column}
...
\vspace{0.45em}
\begin{block}{Terminal reward는 evidence quality를 보지 못함}
\small
...
\node[overlay, anchor=west, align=left, font=\scriptsize\bfseries, text=red!75!black]
  at ([xshift=-0.60cm,yshift=-0.55cm]pan.east)
  {췌장염:\\3개 기준 중\\2개 이상이면 진단};
\end{tikzpicture}
\par\vspace{-0.05em}
...
\node[overlay, anchor=east, align=right, font=\tiny\bfseries, text=red!75!black,
  fill=white, fill opacity=0.88, text opacity=1, inner sep=1pt]
  at ([xshift=-0.18cm,yshift=1.70cm]app.east)
  {blood tests alone으로는\\충수염 진단 불가};
\node[overlay, anchor=east, align=right, font=\tiny\bfseries, text=red!75!black,
  fill=white, fill opacity=0.88, text opacity=1, inner sep=1pt]
  at ([xshift=-0.18cm,yshift=-1.05cm]app.east)
  {증상/진찰 소견이 있을 때\\imaging으로 확정};
\end{tikzpicture}
\par\vspace{-0.05em}
...
\begin{block}{핵심}
\small
좋은 diagnostic path에 대한 직접적인 supervision은 없다. 그래서 LA-CDM은 최종 진단 정확도와 검사 비용을 reward로 두고, 여러 trajectory를 trial-and-error로 시도하면서 test-selection policy를 학습한다.
\end{block}
\end{frame}
