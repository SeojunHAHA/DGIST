...
\vspace{0.3em}
\includegraphics[width=0.86\linewidth,height=0.26\textheight,keepaspectratio]{figures/SIFT/result_logistic_regression.png}
...
\vspace{0.03em}
{\scriptsize\textcolor{gray!70}{Source: American College of Gastroenterology (ACG) Clinical Guideline: Management of Acute Pancreatitis, 2013.}}
...
\includegraphics[
  ...
  keepaspectratio
]{figures/image copy.png}
\end{column}
\hspace{-0.25cm}
\begin{column}{0.33\textwidth}
\begin{block}{Acute pancreatitis guideline 예시}
\footnotesize
급성 췌장염은 보통 \textbf{복통, amylase/lipase 상승, 영상 소견} 중 두 가지 이상이 만족될 때 진단된다.

\vspace{0.18em}
즉, 같은 검사라도 이전 evidence에 따라 marginal value가 달라진다.
\end{block}
\end{column}
\end{columns}
\end{frame}
...
\begin{frame}{기존 방법들의 trajectory-level 접근}
\vspace{0.0em}
\centering
...
\draw[edge] (l3) -- (l4);
\draw[edge] (l4) -- (l5);
\node[note] at (-0.55,0.55) {trajectory \(z=(A,B,C)\)를 만들고};
\node[note] at (3.2,0.55) {끝의 정답 confidence /\\accuracy를 보고 따라감};
...
\draw[edge] (s3) -- (s4);
\draw[edge] (s4) -- (s5);
\node[note] at (-1.25,-2.02) {trajectory \(\tau=\{(a_t,o_t)\}\) 안에서\\critical facts를 얼마나 회수했는지 측정};
\node[font=\tiny, text=TextGray, align=center] at (3.2,-2.42) {\(R(\tau)=(\alpha CR+\beta)acc+\eta CR-\lambda t/T\)};
\end{tikzpicture}

\vspace{0.35em}
\begin{alertblock}{두 연구의 기본 가정}
\small
하나의 검사가 주는 정보량이 비교적 \textbf{독립적}이라고 본다. 즉, 어떤 검사 결과를 이미 알았다는 사실이 다른 검사를 추가했을 때의 예측 변화에 크게 영향을 주지 않는다고 가정하는 셈이다.
\end{alertblock}
\end{frame}

\begin{frame}{Greedy가 실패하는 이유는 중간 reward 부재가 아니다}
\vspace{0.1em}
\begin{columns}[T,onlytextwidth]
\begin{column}{0.48\textwidth}
\centering
...
\begin{alertblock}{의미}
\small
중간 reward가 있어도 \(A\), \(B\)를 각각 따로 평가하면 \(A\)가 더 좋아 보인다. 문제는 reward의 위치가 아니라, 한 검사의 가치가 \textbf{이후 어떤 검사와 결합되는지}에 따라 달라진다는 점이다.
\end{alertblock}
\end{frame}
...
\small
\textbf{설명}\\
검사 간 조건부 의존성을 명시적으로 모델링하고, evidence가 추가될 때 posterior를 업데이트한다.

\vspace{0.55em}
\textbf{장점}
\begin{itemize}
  \item 조합 효과와 redundancy를 원리적으로 다룰 수 있다
  \item query order를 probabilistic하게 정의할 수 있다
\end{itemize}

\vspace{0.25em}
\textbf{단점}
\begin{itemize}
  \item 자연어 report를 discrete variable로 바꾸기 어렵다
  {\scriptsize 예: CT report \(\rightarrow\) severe: yes/no로 줄이면 표현력이 감소}
\end{itemize}
\end{block}
\end{column}
...
\small
\textbf{설명}\\
현재 검사 하나만 보지 않고, \(n\) step 뒤 가능한 조합의 utility를 미리 측정해 학습 신호로 쓴다.

\vspace{0.45em}
\textbf{장점}
\begin{itemize}
  \item 전체 subset을 다 label하지 않아도 된다
  \item 작은 조합의 synergy를 직접 학습시킬 수 있다
\end{itemize}

\vspace{0.2em}
\textbf{단점}
\begin{itemize}
  \item computational cost가 exponential하게 증가한다
\end{itemize}
